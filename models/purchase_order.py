...
class PurchaseOrder(models.Model):
    ...
    inter_transfer_ids = fields.One2many('inter.company.transfer', 'purchase_order_id')
    count_inter_transfer = fields.Integer(string='Count Inter Transfer', compute='compute_count_inter_transfer',
                                          store=True)
            ...
            if not self.env.context.get('skip_auto'):
                ...
                if (self.check_setting_auto_workflow_inter_transaction()
                    ...
                    inter_company_transaction = self.env['inter.company.transfer'].create({
                        'from_warehouse_id': partner_company.intercompany_warehouse_id.id,
                        'to_warehouse_id': rec.company_id.intercompany_warehouse_id.id,
                        'type': 'transfer',
                        'apply_type': 'sale',
                    ...
                    # auto validate picking of Purchase
                    rec.picking_ids.filtered(lambda x: x.state != 'cancel' and x.state == 'assigned').with_context(
                        skip_backorder=True).button_validate()

                    # Create bill for Purchase and post it
                    rec.action_create_invoice()
                    bill = rec.invoice_ids.filtered(lambda x: x.state not in ['posted', 'cancel'])
                    ...
                    # Write bill into transaction
                    inter_company_transaction.bill_id = bill.id
        return res
